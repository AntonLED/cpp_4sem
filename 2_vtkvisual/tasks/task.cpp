...
public:
    ...
    void move(double tau) {
        x += vx * tau + ax * tau * tau / 2;
        y += vy * tau + ay * tau * tau / 2;
        z += vz * tau + az * tau * tau / 2;
    }
};
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = -100 + static_cast<float>(rand()) / (static_cast<float>(RAND_MAX/(200)));
    ...
    void doTimeStep(double tau) {
        for (unsigned i = 0; i < nodes.size(); i++) {
            nodes[i].smth = -100 + static_cast<float>(rand()) / (static_cast<float>(RAND_MAX/(200)));

            double prev_x = nodes[i].x; 
            double prev_y = nodes[i].y; 
            double prev_z = nodes[i].z; 

            nodes[i].x = 
                (cos(omega_a*M_PI*tau)*cos(omega_g*M_PI*tau) - cos(omega_b*M_PI*tau)*sin(omega_a*M_PI*tau)*sin(omega_g*M_PI*tau)) * prev_x +
                (-cos(omega_g*M_PI*tau)*sin(omega_a*M_PI*tau) - cos(omega_a*M_PI*tau)*cos(omega_b*M_PI*tau)*sin(omega_g*M_PI*tau)) * prev_y + 
                sin(omega_b*M_PI*tau)*sin(omega_g*M_PI*tau) * prev_z; 
            nodes[i].y = 
                (cos(omega_b*M_PI*tau)*cos(omega_g*M_PI*tau)*sin(omega_a*M_PI*tau) + cos(omega_a*M_PI*tau)*sin(omega_g*M_PI*tau)) * prev_x + 
                (cos(omega_a*M_PI*tau)*cos(omega_b*M_PI*tau)*cos(omega_g*M_PI*tau) - sin(omega_a*M_PI*tau)*sin(omega_g*M_PI*tau)) * prev_y -
                cos(omega_g*M_PI*tau) * sin(omega_b*M_PI*tau) * prev_z; 
            nodes[i].z = 
                sin(omega_a*M_PI*tau)*sin(omega_b*M_PI*tau) * prev_x + 
                cos(omega_a*M_PI*tau)*sin(omega_b*M_PI*tau) * prev_y + 
                cos(omega_b*M_PI*tau) * prev_z; 
               
            nodes[i].vx = (nodes[i].x - prev_x) / tau;
            nodes[i].vy = (nodes[i].y - prev_y) / tau; 
            nodes[i].vz = (nodes[i].z - prev_z) / tau;  
        }

        omega_a = static_cast<float>(rand()) / (static_cast<float>(RAND_MAX/(100)));
        omega_b = static_cast<float>(rand()) / (static_cast<float>(RAND_MAX/(100)));
        omega_g = static_cast<float>(rand()) / (static_cast<float>(RAND_MAX/(100)));
    }
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "./vtu_frames/task-step-" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    ...
    // Считаем STL
    try {
        gmsh::merge("diamond_no_girdle.stl");
    } catch(...) {
